...
//　1.0.2 ：2025/10/29　：setDropDown引数追加
//★★★★★★★★★★★★★★★★★★★★★★★★★★★
/** 自アプリのフィールド情報を取得
 ...
 * @param {boolean} [allField=false] trueの場合、サブテーブルの内外問わず全てのフィールドを対象とする
 * @param {string} [subtableCode=null] サブテーブル内のフィールドの場合、対象となるサブテーブルのフィールド名
 * @returns {object} ドロップダウン作成用の配列
 */
//★★★★★★★★★★★★★★★★★★★★★★★★★★★
export function setDropDown(fieldArray, allField = false, subtableCode = null) {
  const result = [];
  for (const field of fieldArray) {
    ...
    if (allField) {
      if (field.hasOwnProperty('subtableCode')) {
        addItem.id += ',' + field.subtableCode; //サブテーブルの場合、「フィールド名,サブテーブル名」
        addItem.name += '(' + escapeHtml(field.subtableCode) + ')'; //ドロップダウンへの表示内容：サブテーブルの場合、「フィールド名(サブテーブル名)」
      }
      result.push(addItem);
    } else {
      if (!field.hasOwnProperty('subtableCode')) {
